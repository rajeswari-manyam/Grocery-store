  ...
  return (
    <AnimatePresence>
      {open && (
        ...
        >
          {/* Top bar */}
          <div className="relative z-20 bg-white flex items-center gap-3 px-4 py-3 shadow-sm">
            <button onClick={onClose} className="p-1.5 rounded-full hover:bg-slate-100 bg-transparent border-none cursor-pointer flex-shrink-0">
          ...
          <div className="flex-1 relative overflow-hidden">
            {loading ? (
              ...
              <MapContainer center={center} zoom={17} className="w-full h-full z-0" scrollWheelZoom={true} zoomControl={false}>
                <TileLayer
            ...
            <button
              ...
              disabled={geoLoading}
              className="absolute bottom-4 right-4 z-[1000] w-11 h-11 rounded-full bg-white shadow-lg border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-all disabled:opacity-50 border-none cursor-pointer"
            >
              <Crosshair size={18} className={geoLoading ? 'animate-spin text-emerald-600' : 'text-slate-600'} />
            </button>
          ...
          {/* Bottom sheet */}
          <div className="bg-white px-5 pt-4 pb-6 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]">
            {resolving ? (
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl bg-emerald-700 hover:bg-emerald-800 disabled:bg-slate-300 text-white font-semibold text-sm transition-all border-none cursor-pointer flex items-center justify-center gap-2 shadow-sm"
            >
              <Navigation size={16} />
              Confirm Location
            </button>
